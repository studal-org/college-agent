...
Функция Обработчик(Контекст) Экспорт
	...
	ВидНагрузки = stdl_ВидыНагрузки.Получить(Новый УникальныйИдентификатор(Параметры.id));

	Если ВидНагрузки = Неопределено Тогда
		ТелоОтвета = stdl_Ошибки.ReferenceNotFound("WorkType with provided ID was not found.", "workType");
		Возврат Ответ_404_application_json(ТелоОтвета);
	КонецЕсли;

	ТелоОтвета = stdl_Модели.WorkType(ВидНагрузки);
