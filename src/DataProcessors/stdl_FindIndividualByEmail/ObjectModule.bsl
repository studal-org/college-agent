...
Функция Обработчик(Контекст) Экспорт
	...
	ФизическоеЛицо = stdl_ФизическиеЛица.ПолучитьПоЭлектроннойПочте(Параметры.email);

	Если ФизическоеЛицо = Неопределено Тогда
		ТелоОтвета = stdl_Ошибки.NotFound("Individual with provided email was not found.");
	...
	Если Администратор = Неопределено И Студент = Неопределено Тогда
		ТелоОтвета = stdl_Ошибки.NotFound("This Individual is neither an Administrator nor a Student.");
		Возврат Ответ_404_application_json(ТелоОтвета);
	КонецЕсли;
