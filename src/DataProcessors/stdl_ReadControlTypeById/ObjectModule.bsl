...
Функция Обработчик(Контекст) Экспорт
	...
	ВидКонтроля = stdl_ВидыКонтроля.Получить(Новый УникальныйИдентификатор(Параметры.id));

	Если ВидКонтроля = Неопределено Тогда
		ТелоОтвета = stdl_Ошибки.ReferenceNotFound("ControlType with provided ID was not found.", "controlType");
		Возврат Ответ_404_application_json(ТелоОтвета);
	КонецЕсли;

	ТелоОтвета = stdl_Модели.ControlType(ВидКонтроля);
...
//		*id - Строка -
//		*title - Строка -
//		*markTypeId - Неопределено, Строка -
// Возвращаемое значение:
//	Неопределено
