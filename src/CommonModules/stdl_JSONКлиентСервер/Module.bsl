// Преобразовать невалидный тип JSON.
// 
// Параметры:
//  Значение - Произвольный
// 
// Возвращаемое значение:
//  Неопределено - Преобразовать невалидный тип JSON
//&НаСервере
//Функция ПреобразоватьНевалидныйТипJSON(Свойство, Значение, ДополнительныеПараметры, Отказ) Экспорт
...
Функция jsonЗаписатьИнициализация(Значение, Знач Стандарт, Знач ПредставленияСсылок)
	...
	#Если ВебКлиент Тогда
			Массивы = Новый ОписаниеТипов("Массив,ФиксированныйМассив");														// Массивы.
			Стуктуры = Новый ОписаниеТипов("Структура,ФиксированнаяСтруктура");													// Структуры.
			Соответствия = Новый ОписаниеТипов("Структура,ФиксированнаяСтруктура,Соответствие,ФиксированноеСоответствие");		// Соответсвия.
	#Иначе
		...
		#Иначе
			#Если НаСервере Тогда
				Массивы = Новый ОписаниеТипов("Массив,ФиксированныйМассив,ФиксированнаяКоллекция");						// Массивы.
				Стуктуры = Новый ОписаниеТипов("Структура,ФиксированнаяСтруктура");										// Структуры.
				Соответствия = Новый ОписаниеТипов("Структура,ФиксированнаяСтруктура,Соответствие,ФиксированноеСоответствие");	// Соответсвия.
			#Иначе
				Массивы = Новый ОписаниеТипов("Массив,ФиксированныйМассив");												// Массивы.
				Стуктуры = Новый ОписаниеТипов("Структура");																// Структуры.
				Соответствия = Новый ОписаниеТипов("Структура,Соответствие");													// Соответсвия.
			#КонецЕсли
		#КонецЕсли
	#КонецЕсли
	
	#Если ВебКлиент Тогда
		Построители = Неопределено;
	#Иначе
		#Если ТонкийКлиент Тогда
			Построители = Неопределено;																						// Построители.
		#Иначе
			Построители = Новый ОписаниеТипов("ПостроительЗапроса,ПостроительОтчета");										// Построители.
		#КонецЕсли																					// Построители.										// Построители.
	#КонецЕсли
	
	#Если НаСервере Тогда
		ТипДопустимыхКлючей = Новый ОписаниеТипов("ДанныеФормыКоллекция,ДанныеФормыСтруктураСКоллекцией,ДанныеФормыДерево");		// КлиентскиеТипы.
	#Иначе
		ТипДопустимыхКлючей = Неопределено;																						// КлиентскиеТипы.
	#КонецЕсли
	...
	ВспомогательныеДанные = Новый Структура("ПримитивныеТипы,Массивы,Структуры,Соответсвия,Построители,КлиентскиеТипы,ТипДопустимыхКлючей", 
		...
		Массивы, Стуктуры, Соответствия, Построители, ТипДопустимыхКлючей,
